...
def elbow_curve(x_train):
	...
	plt.scatter(7, score[6],facecolors='none',edgecolors='r',s=200,linewidth=2)
...
def visualize_labels(column_scales, column_min, x_data, y_data):
	kmeans = KMeans(n_clusters=7).fit(x_data)
	labels = kmeans.labels_

	labels_mapped, prediction_clusters, prediction_clusters_rounded = map_labels(y_data, kmeans)
	print("Informal homogeneity per cluster : " + str(prediction_clusters))

	view_data(column_scales, column_min, x_data, labels, labels_mapped)
...
def map_labels(y_data, clf):
	...
	tot_samples = [0 for i in range(7)]
	predicted_samples = [0 for i in range(7)]

	for i in range(len(labels)):
		tot_samples[labels[i]] += 1
		predicted_samples[labels[i]] += y_data[i]

	prediction_clusters = [predicted_samples[i]/tot_samples[i] for i in range(7)]
	prediction_clusters_rounded = [round(val) for val in prediction_clusters]
	labels_mapped = [prediction_clusters_rounded[label] for label in labels]

	print(len(labels_mapped))	
...
def main():
	...
	# #VISUALIZE CLUSTERS
	# visualize_labels(column_scales, column_min, x_train, y_train)
